...
function ResourceDisplay:init(x, y, scale, init_arcane, init_hemo, init_holy, init_unholy)
    local resource_display = {
        ...
        arcane = init_arcane or 0,
        hemo = init_hemo or 0,
        holy = init_holy or 0,
        unholy = init_unholy or 0
    }
    ...
    resource_display.addResource = function(card_type, num)
        ...
        if card_type == "arcane" then
            ...
            resource_display.arcane = resource_display.arcane + num > RESOURCE_CAP and RESOURCE_CAP or resource_display.arcane + num
        elseif card_type == "hemo" then
            resource_display.hemo = resource_display.hemo + num > RESOURCE_CAP and RESOURCE_CAP or resource_display.hemo + num
        elseif card_type == "holy" then
            resource_display.holy = resource_display.holy + num > RESOURCE_CAP and RESOURCE_CAP or resource_display.holy + num
        elseif card_type == "unholy" then
            resource_display.unholy = resource_display.unholy + num > RESOURCE_CAP and RESOURCE_CAP or resource_display.unholy + num
        end
    end
    ...
    resource_display.subtractResource = function(card_type, num)
        if num < 0 then error "num cannot be negative" end
        if card_type == "arcane" then
            resource_display.arcane = resource_display.arcane - num < 0 and 0 or resource_display.arcane - num
        elseif card_type == "hemo" then
            resource_display.hemo = resource_display.hemo - num < 0 and 0 or resource_display.hemo - num
        elseif card_type == "holy" then
            resource_display.holy = resource_display.holy - num < 0 and 0 or resource_display.holy - num
        elseif card_type == "unholy" then
            resource_display.unholy = resource_display.unholy - num < 0 and 0 or resource_display.unholy - num
        end
    end
    ...
    resource_display.draw = function()
        ...
        local text_x = resource_display.x + offset_x / 2 - font:getWidth(""..resource_display.arcane) / 2
        ...
        if resource_display.arcane == RESOURCE_CAP then
            love.graphics.setColor(Colors.red)
            love.graphics.print(""..resource_display.arcane, font, text_x, text_y)
            love.graphics.setColor(Colors.white)
        else love.graphics.print(""..resource_display.arcane, font, text_x, text_y) end
        ...
        if resource_display.hemo == RESOURCE_CAP then
            love.graphics.setColor(Colors.red)
            love.graphics.print(""..resource_display.hemo, font, text_x, text_y)
            love.graphics.setColor(Colors.white)
        else love.graphics.print(""..resource_display.hemo, font, text_x, text_y) end
        resource_x = resource_x + offset_x
        text_x = text_x + offset_x

        love.graphics.draw(HOLY_IMG, resource_x, resource_y, 0, resource_scale, resource_scale)
        if resource_display.holy == RESOURCE_CAP then
            love.graphics.setColor(Colors.red)
            love.graphics.print(""..resource_display.holy, font, text_x, text_y)
            love.graphics.setColor(Colors.white)
        else love.graphics.print(""..resource_display.holy, font, text_x, text_y) end
        resource_x = resource_x + offset_x
        text_x = text_x + offset_x

        love.graphics.draw(UNHOLY_IMG, resource_x, resource_y, 0, resource_scale, resource_scale)
        if resource_display.unholy == RESOURCE_CAP then
            love.graphics.setColor(Colors.red)
            love.graphics.print(""..resource_display.unholy, font, text_x, text_y)
            love.graphics.setColor(Colors.white)
        else love.graphics.print(""..resource_display.unholy, font, text_x, text_y) end
        resource_x = resource_x + offset_x
        text_x = text_x + offset_x
        love.graphics.pop()
    end
